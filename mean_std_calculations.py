        ...
        for (strategy, folders) in zip(strategies, extra_log_folders):
            for folder in folders:
                ...
                mean_std_df = wrapper(
                    ...
                    args.task, outputs, strategy, extra_log_folder, metric=metric
                )
                if mean_std_df is not None:
                    log_folder = get_log_folder(
                        args.pow_type, args.cluster_type, args.task, args.dataset_type,
                        outputs, strategy, extra_log_folder, count=0, task_id=0,
                    ...
                    times[f'{strategy} - {simulator_prefix}/{folder}'] = total_time            
                queue.update(1)
    print("Times: ")
