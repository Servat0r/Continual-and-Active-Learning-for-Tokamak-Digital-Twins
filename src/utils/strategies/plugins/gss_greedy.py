...
class RegressionGSS_greedyPlugin(SupervisedPlugin):
    """
    A variant of Avalanche GSS_greedyPlugin for regression problems with multiple outputs.
    """

    def __init__(
            self, mem_size=200, mem_strength=5,
            input_size=[], output_size=[], update_every: int = 1
    ):
        """
        ...
        self.ext_mem_list_y = torch.FloatTensor(mem_size, *output_size).fill_(0)
    ...
    def get_rand_mem_grads(self, strategy, grad_dims, gss_batch_size):
        ...
        num_mem_subs = min(
            self.mem_strength, self.ext_mem_list_current_index // temp_gss_batch_size
        )
    ...
    def before_training_exp(self, strategy, num_workers=0, shuffle=True, **kwargs):
        ...
        self._current_epoch = 0
        stdout_debug_print(
            f"Before training exp {strategy.experience.current_experience}, we have: " + \
            f"\nX_mem shape = {self.ext_mem_list_x.shape}, \nY_mem shape = {self.ext_mem_list_y}",
            color='green'
        )
        ...
        raw_csv_dataset = strategy.experience.dataset._datasets[0]
        transform = raw_csv_dataset.transform
        target_transform = raw_csv_dataset.target_transform
        memory_dataset = AvalancheDataset(
            [CSVRegressionDataset(
                data=None, input_columns=None, output_columns=None,
                inputs=self.ext_mem_list_x, outputs=self.ext_mem_list_y,
                transform=transform, target_transform=target_transform
            )]
        )
        #memory_dataset = _make_taskaware_classification_dataset( # TODO CHANGE!
    ...
    def after_forward(self, strategy: SupervisedTemplate, num_workers=0, shuffle=True, **kwargs):
        """
        After every forward this function select sample to fill
        the memory buffer based on cosine similarity
        """
        if self._current_epoch % self.update_every:
            return
        #stdout_debug_print(
        #    f"After forward after epoch {self._current_epoch} (update every {self.update_every} epoch)",
        #    color='cyan'
        #)
...
__all__ = ['RegressionGSS_greedyPlugin']
