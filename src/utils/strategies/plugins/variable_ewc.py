...
class VariableEWCPlugin(SupervisedPlugin):
    """
    Variation of EWCPlugin that allows for dynamic lambda values over experiences.
    """
    ...
    def __init__(
        ...
        self.ewc_lambda_list = ewc_lambda if isinstance(ewc_lambda, Sequence) else [ewc_lambda]
        self.ewc_lambda = self.ewc_lambda_list[0]
        self.mode = mode
    ...
    def after_training_exp(self, strategy, **kwargs):
        ...
        """
        exp_counter = strategy.experience.current_experience
        ...
        if len(self.ewc_lambda_list) > 1: # More than one lambda
            self.ewc_lambda = self.ewc_lambda_list[exp_counter]
